...
let private maxVersionWithThreshold (minVersion: NugetVersion) (versions: NugetVersion []) =
    versions
    |> Array.filter (fun v -> compareNugetVersion v minVersion >= 0) // get all versions that compare as greater than the minVersion
    |> Array.sortWith compareNugetVersion
    |> Array.tryLast
...
let latest3xSdkVersion sdkRoot =
    let minSDKVersion = NugetVersion(3,0,100,"")
    lazy (
        match sdkVersions sdkRoot with
        | None -> None
...
let latest3xRuntimeVersion sdkRoot =
    let minRuntimeVersion = NugetVersion(3,0,0,"")
    lazy (
        match runtimeVersions sdkRoot with
        | None -> None
...
let getRefs () =
    let sdkVersion = NugetVersion(8,0,102, "") //latest8xSdkVersion defaultDotNetSDKRoot
    let runtimeVersion = NugetVersion(8,0,0, "")//latest8xRuntimeVersion defaultDotNetSDKRoot
